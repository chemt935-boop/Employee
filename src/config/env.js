...
const schema = Joi.object({
  ...
  DB_TRUST_SERVER_CERT: Joi.boolean()
    .truthy('true')
    .falsy('false')
    .default(true),
  ...
  EMAIL_RETURN_TOKEN_IN_RESPONSE: Joi.boolean()
    .truthy('true')
    .falsy('false')
    .default(false),

  // Resend
  RESEND_API_KEY: Joi.string().allow('', null).optional(),
...
function loadEnv(processEnv) {
  const { value, error } = schema.validate(processEnv, {
    abortEarly: false
  });
